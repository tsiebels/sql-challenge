...
### Challenge Presented
The following Spark SQL query runs against a large production data warehouse and is timing out. Identify all performance issues and rewrite it as an optimized query.
...
### Challenge Presented
A Spark job is taking 45 minutes to complete. Review the EXPLAIN output below. Identify the core bottleneck and explain specifically how you would resolve it.
...
### Challenge Presented
Write a production-quality, optimized Spark SQL query for the business requirement below. Apply all relevant Spark-specific performance considerations.

**Requirements & Table Context:**
- Requirement: Return each store's daily total revenue AND 7-day rolling average revenue for Q1 2024 (Jan 1 – Mar 31) only.
- sales — sale_date DATE [partitioned], store_id STRING, revenue DECIMAL(12,2)
- store_lookup — ~500 rows: store_id STRING, region STRING, store_name STRING
...
### Part 1 — Query Optimization (35 pts)
- **correctness (0–15):** Rewritten query produces correct results with explicit JOIN syntax replacing implicit comma joins
- **spark_optimization (0–20):** Broadcast hints for small tables (customers, products); partition pruning via date range instead of YEAR() wrapping; SELECT specific columns not SELECT *; modern explicit JOIN syntax

### Part 2 — Execution Plan Analysis (25 pts)
- **diagnosis (0–15):** Identified that the 42 MB customers table should use BroadcastHashJoin not SortMergeJoin; the 14.2 GB shuffle is the bottleneck; root cause is join strategy selection
- **clarity (0–10):** Technically precise; actionable fix stated (BROADCAST hint or spark.sql.autoBroadcastJoinThreshold config)

### Part 3 — Write From Scratch (40 pts)
- **correctness (0–15):** Daily revenue per store + 7-day rolling avg via window function (AVG OVER ROWS BETWEEN 6 PRECEDING AND CURRENT ROW); correct Q1 2024 date filter
- **spark_optimization (0–15):** Broadcast hint on store_lookup; date range filter for partition pruning (sale_date BETWEEN, not YEAR()/MONTH() wrappers); selects only needed columns
- **code_quality (0–10):** CTEs or clean structure; readable formatting; production-ready style
...
---
*Generated by BI Engineering Evaluation Tool*`;
};
